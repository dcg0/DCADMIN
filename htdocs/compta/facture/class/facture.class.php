...
class Facture extends CommonInvoice
{
	 ...
	 * - CLOSECODE_BANKCHARGE = 'bankcharge'
	 * - CLOSECODE_WITHHOLDINGTAX = 'withholdingtax'
	 * - CLOSECODE_OTHER = 'other'
	 * - CLOSECODE_ABANDON = 'abandon'
	 *
	 * - CLOSECODE_REPLACED = 'replaced'
	 * - All constant CLOSECODE_XXX are into commoninvoice.class.php
	 *
	 ...
	 * $this->close_code can beseveral values.
	 * - This constant CLOSECODE_XXX into commoninvoice.class.php
	 */
			...
			if ($this->type != self::TYPE_DEPOSIT && $result >= 0 && isModEnabled('stock') && getDolGlobalString('STOCK_CALCULATE_ON_BILL') && $idwarehouse != -1) {
				...
				for ($i = 0; $i < $num; $i++) {
					if ($this->lines[$i]->fk_product > 0) {
						...
						if ($this->type == self::TYPE_CREDIT_NOTE) {
							$result = $mouvP->livraison($user, $this->lines[$i]->fk_product, $idwarehouse, $this->lines[$i]->qty, $this->lines[$i]->subprice, $langs->trans("InvoiceDeleteDCADMIN", $this->ref));
						} else {
							$result = $mouvP->reception($user, $this->lines[$i]->fk_product, $idwarehouse, $this->lines[$i]->qty, 0, $langs->trans("InvoiceDeleteDCADMIN", $this->ref)); // we use 0 for price, to not change the weighted average value
						}
					}
				}
			}
				...
				for ($i = 0; $i < $cpt; $i++) {
					if ($this->lines[$i]->fk_product > 0) {
						...
						if ($this->type == self::TYPE_CREDIT_NOTE) {
							...
							$result = $mouvP->reception($user, $this->lines[$i]->fk_product, $idwarehouse, $this->lines[$i]->qty, 0, $langs->trans("InvoiceValidatedInDCADMIN", $num), '', '', $this->lines[$i]->batch);
							if ($result < 0) {
								$error++;
								...
								if ($productStatic->hasbatch() && is_object($productbatch)) {
									...
									if (!$error) {
										...
										foreach ($batchList as $batch) {
											...
											$result = $mouvP->livraison($user, $productStatic->id, $idwarehouse, $product_batch_qty, $this->lines[$i]->subprice, $langs->trans('InvoiceValidatedInDCADMIN', $num), '', '', '', $batch->batch);
											if ($result < 0) {
												$error++;
												$this->setErrorsFromObject($mouvP);
												break;
										...
										if (!$error && $product_qty_remain > 0) {
											if (getDolGlobalInt('STOCK_DISALLOW_NEGATIVE_TRANSFER')) {
												...
												$batch = $batchList[0];
												$result = $mouvP->livraison($user, $productStatic->id, $idwarehouse, $product_qty_remain, $this->lines[$i]->subprice, $langs->trans('InvoiceValidatedInDCADMIN', $num), '', '', '', $batch->batch);
												if ($result < 0) {
													$error++;
													$this->setErrorsFromObject($mouvP);
												}
							...
							if (!$is_batch_line) {	// If stock move not yet processed
								$result = $mouvP->livraison($user, $this->lines[$i]->fk_product, $idwarehouse, $this->lines[$i]->qty, $this->lines[$i]->subprice, $langs->trans("InvoiceValidatedInDCADMIN", $num));
								if ($result < 0) {
									$error++;
									$this->setErrorsFromObject($mouvP);
								}
			...
			if ($this->type != self::TYPE_DEPOSIT && $result >= 0 && isModEnabled('stock') && getDolGlobalString('STOCK_CALCULATE_ON_BILL')) {
				...
				for ($i = 0; $i < $num; $i++) {
					if ($this->lines[$i]->fk_product > 0) {
						...
						// We decrease stock for product
						if ($this->type == self::TYPE_CREDIT_NOTE) {
							$result = $mouvP->livraison($user, $this->lines[$i]->fk_product, $idwarehouse, $this->lines[$i]->qty, $this->lines[$i]->subprice, $langs->trans("InvoiceBackToDraftInDCADMIN", $this->ref));
						} else {
							$result = $mouvP->reception($user, $this->lines[$i]->fk_product, $idwarehouse, $this->lines[$i]->qty, 0, $langs->trans("InvoiceBackToDraftInDCADMIN", $this->ref)); // we use 0 for price, to not change the weighted average value
						}
					}
				}
			}
