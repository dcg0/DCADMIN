	...
	if (array_key_exists($key, $object->fields)) {
		...
		if (empty($field_spec['searchmulti'])) {
			...
			if ($search[$key] != '') {
				...
				} elseif (is_array($search[$key]) && !empty($search[$key])) {
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".DOL_URL_ROOT.'/compta/cashcontrol/cashcontrol_card.php?id='.((int) $id));
	exit;
}
...
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'mod-takepos page-list bodyforlist');
...
if ($search_all) {
	...
	print '<!-- Search done like if CASHFENCE_QUICKSEARCH_ON_FIELDS = '.$setupstring.' -->'."\n";
