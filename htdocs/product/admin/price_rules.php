...
if ($action == 'update') {
	...
	for ($i = 1; $i <= $produit_multiprices_limit; $i++) {
		...
		$sql .= ((int) $i).", ".((int) $i_fk_level).", ".((float) $i_var_percent).", ".((float) $i_var_min_percent).")";

		if (!$db->query($sql)) {
			//If we could not create, then we try updating
			$sql = "UPDATE ".MAIN_DB_PREFIX."product_pricerules";
			$sql .= " SET fk_level = ".((int) $i_fk_level).", var_percent = ".((float) $i_var_percent).", var_min_percent = ".((float) $i_var_min_percent)." WHERE level = ".((int) $i);
