...
class SocialContributions extends DCADMINApi
{
	...
	public function getAllPayments($sortfield = "t.rowid", $sortorder = 'ASC', $limit = 100, $page = 0)
	{
		...
		$sql = "SELECT t.rowid FROM ".MAIN_DB_PREFIX."paiementcharge AS t, ".MAIN_DB_PREFIX."chargesociales AS s";
		$sql .= " WHERE t.fk_charge = s.rowid AND s.entity IN (".getEntity('chargesociales').")";
		$sql .= $this->db->order($sortfield, $sortorder);
		if ($limit) {
			if ($page < 0) {
				$page = 0;
