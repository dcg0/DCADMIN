...
$experimentName = 'UxDCADMINContextHowItWork';
...
<div class="doc-wrapper">
	...
	<div class="doc-content-wrapper">
		...
		<h1 class="documentation-title"><?php echo $langs->trans($group); ?> : <?php echo $langs->trans('UxDCADMINContextHowItWork'); ?></h1>
		...
		<div class="documentation-section">
			...
			<p>
				DCADMINContext is a secure global JavaScript context for DCADMIN.
				It provides a single global object <code>window.DCADMIN</code>, which cannot be replaced.
			...
			<p>
				...
				that encapsulate functionality such as standardized AJAX requests and responses, ensuring consistent data handling across DCADMIN modules.
			...
			<p>
				Beyond DOM-based events, DCADMINContext allows monitoring and reacting to business events using a
				hook-like mechanism. For instance, you can listen to events such as
				<code>DCADMIN.on('addline:load:productPricesList', function(data) { ... });</code>
				without relying on DOM changes. This enables creating logic that reacts directly to application state changes.
			</p>

			<p>
				Similarly, you can define tools that act as global helpers, like <code>DCADMIN.tools.setEventMessage()</code>.
				This tool can display notifications (similar to PHP's <code>setEventMessage()</code> in DCADMIN),
			...
			<p>
				In summary, DCADMINContext provides a secure, extensible foundation for adding tools, monitoring business events,
				and standardizing interactions across DCADMIN's frontend modules.
			</p>
		</div>
		...
		<div class="documentation-section">
			...
			<p>
				...
				If the help does not appear automatically, type <code>DCADMIN.tools.showConsoleHelp();</code> in the console to display it.
			</p>
		</div>

		<div class="documentation-section">
			<h2 class="documentation-title">DCADMIN.log() and debugMode()</h2>

			<p>
				<code>DCADMIN.log()</code> is a lightweight logging utility provided by the DCADMIN JS context.
				It does <strong>not</strong> replace <code>console.log()</code>, but it gives an important advantage:
				you can enable or disable all logs globally using <code>DCADMIN.debugMode()</code>.
			</p>

			<h3>Why use DCADMIN.log() instead of console.log()?</h3>
			<ul>
			...
			<h3>How it works</h3>
			<p>
				When <code>debugMode</code> is disabled (default state), calls to <code>DCADMIN.log()</code> do nothing.
				When enabled, <code>DCADMIN.log()</code> behaves like <code>console.log()</code>.
			</p>
				...
				$lines = array(
					...
					'	DCADMIN.log("My debug message");',
					'',
					'	// Enable verbose logs',
					'	DCADMIN.debugMode(true);',
					'',
					'	// Disable logs again',
					'	DCADMIN.debugMode(false);',
					'</script>',
				);
			...
			<ul>
				...
				<li><code>DCADMIN.log()</code> → prints messages only when debug mode is ON; fully controllable. Ideal for DCADMIN core logs or when you want to keep logs available but silent in production.</li>
				<li><code>DCADMIN.debugMode(true)</code> → enable verbose logs, activating <code>DCADMIN.log()</code> output.</li>
				<li><code>DCADMIN.debugMode(false)</code> → disable all <code>DCADMIN.log()</code> output, silencing debug messages.</li>
			</ul>
		...
		<div class="documentation-section">
			<h2 id="titlesection-hooks"  class="documentation-title">JS DCADMIN hooks</h2>

			<p>
				DCADMIN provides a hook system to allow modules and scripts to communicate with each other
				through named events. There are two ways to listen to these events in JS:
				<code>DCADMIN.on()</code> and <code>document.addEventListener()</code>.
			</p>

			<h3>Event listener example</h3>
			<p>
				You can use <code>DCADMIN.on()</code> to listen to a hook. The main difference with standard
			...
			<p>
				...
				<code>e.detail</code> and the event name is prefixed by <code>DCADMIN:</code> so for a hook named A event name is <code>DCADMIN:A</code>
			</p>

				<?php
				$lines = array(
					'<script>',
					'	// Add a listener to the DCADMIN theEventName event',
					'	DCADMIN.on(\'theEventName\', function(data) {',
					'		console.log(\'DCADMIN theEventName\', data);',
					'	});',
					'',
					'	// But this  work too on document',
					'	document.addEventListener(\'DCADMIN:theEventName\', function(e) {',
					'		console.log(\'DCADMIN theEventName\', e.detail);',
					'	});',
					'</script>',
				);
				$documentation->showCode($lines, 'php'); ?>
			...
			<p>
				When DCADMIN is ready (DOM loaded and JS context initialized), you can register your hooks
				or trigger them. Both <code>DCADMIN.on()</code> and <code>document.addEventListener()</code>
				are valid, but <code>DCADMIN.on()</code> is simpler and more convenient because you get the
				data directly.
			</p>
				<?php
				$lines = array(
					...
					'		// the dom is ready and you are sure DCADMIN js context is loaded',
					'		...',
					'		// Do your stuff',
					'		...',
					'',
					'		// Add a listener to the yourCustomHookName event with dolibarr.on()',
					'		DCADMIN.on(\'yourCustomHookName\', function(data) {',
					'			console.log(\'With DCADMIN.on : data will contain { data01: \'stuff\', data02: \'other stuff\' }\', data);',
					'		});',
					'',
					'		// Or you can do : Add a listener to the yourCustomHookName document.addEventListener()',
					'		document.addEventListener(\'DCADMIN:yourCustomHookName\', function(e) {',
					...
					'			DCADMIN.executeHook(\'yourCustomHookName\', { data01: \'stuff\', data02: \'other stuff\' })',
					'		});',
					'',
					'		...',
					'		// Do your stuff',
		...
		<div  id="titlesection-event-init-vs-ready" class="documentation-section">
			<h2 class="documentation-title">Difference between DCADMIN:Init and DCADMIN:Ready event</h2>

			<p>
				DCADMIN provides two main initialization events for its JavaScript context: <code>DCADMIN:Init</code> and <code>DCADMIN:Ready</code>.
				Understanding their difference is important when developing modules or tools.
			</p>

			<ul>
				<li>
					<strong>DCADMIN:Init</strong> is triggered immediately when the DCADMIN context is created.
					This event is intended for:
					<ul>
						<li>Defining or registering new tools via <code>DCADMIN.defineTool()</code>.</li>
						<li>Setting context variables (<code>DCADMIN.setContextVar()</code> / <code>DCADMIN.setContextVars()</code>).</li>
						<li>Preparing configuration that must be available before the DOM is fully loaded.</li>
					</ul>
					It occurs <em>before</em> <code>DCADMIN:Ready</code>, so it is ideal for setup tasks that other tools may depend on.
				</li>

				<li>
					<strong>DCADMIN:Ready</strong> is triggered once the DOM is ready, similar to <code>$(document).ready()</code> in jQuery.
					This event is intended for:
					<ul>
			...
			<p>
				In short, use <code>DCADMIN:Init</code> for setting up tools and context variables, and <code>DCADMIN:Ready</code> for code that needs the DOM and fully initialized context.
			</p>
			...
			<h3>Examples of usage</h3>
				...
				$lines = array(
					'<script>',
					'	// Example: DCADMIN:Init - define a tool and set context variables early',
					'	document.addEventListener(\'DCADMIN:Init\', function(e) {',
					'		console.log("Init event fired, DCADMIN is initialised and receive context vars a tools");',
					'	});',
					'',
					'	// Example: DCADMIN:Ready - interact with DOM and use tools',
					'	document.addEventListener(\'DCADMIN:Ready\', function(e) {',
					...
					'				alert("Button clicked! Context value: " + DCADMIN.getContextVar("mySetting"));',
					'			});',
					'		}',
					'	});',
					'</script>',
			...
			<p>
				In summary:
			<ul>
				<li><code>DCADMIN:Init</code> → early setup, tools, context variables, configuration.</li>
				<li><code>DCADMIN:Ready</code> → DOM is ready, safe to manipulate elements and use tools defined in Init.</li>
			</ul>
			</p>
		</div>
		...
		<div class="documentation-section">
			...
			<p>
				DCADMIN supports <strong>asynchronous hooks</strong> using <code>DCADMIN.onAwait()</code> and <code>DCADMIN.executeHookAwait()</code>.
			...
			<p>
				Unlike standard synchronous hooks registered with <code>DCADMIN.on()</code>, await hooks return a <code>Promise</code> when executed.
				...
				$lines = array(
					...
					'        DCADMIN.onAwait(\'calculateDiscount\', async function(order) {',
					...
					'        let plus10HookItemId = DCADMIN.onAwait(\'calculateDiscount\', async function(order) {',
					...
					'            order = await DCADMIN.executeHookAwait(\'calculateDiscount\', order);',
		...
		<div class="documentation-section">
			...
			<p>
				The <strong>initNewContent</strong> event is a standardized DCADMIN mechanism
			...
			<p>
				In a typical DCADMIN page, tooltips are initialized on page load for all elements
				...
				<script nonce="<?php print getNonce() ?>">
					document.addEventListener('DCADMIN:Ready', function(e) {
						...
						document.getElementById('try-initNewContent').addEventListener('click', function() {
							...
							DCADMIN.initNewContent(container);
						});
					});
				</script>
			...
			<?php
				$lines = array(
					...
					'        DCADMIN.initNewContent($("#myContainer"), applyToChildrenOnly);',
					'',
					'        // Trigger initNewContent manually on a vanilla JS element',
					'        const element = document.getElementById("myContainer");',
					'        DCADMIN.initNewContent(element, applyToChildrenOnly);',
					'    });',
					'</script>',
				);
				$documentation->showCode($lines, 'php'); ?>
			...
			<p>
				DCADMIN provides a custom event system to properly initialize dynamic content.
		...
		<div class="documentation-section">
			...
			<p>
				You can define reusable and protected tools in the DCADMIN context using <code>DCADMIN.defineTool</code>.
			</p>
			<p>See also <code>dolibarr-context.mock.js</code> for defining all standard DCADMIN tools and creating mock implementations to improve code completion and editor support.</p>
			<p><b>Note :</b> a tool can be a class not only a function</p>
				...
				'<script>',
					...
					'	DCADMIN.defineTool(\'alertUser\', (msg) => alert(\'[DCADMIN] \' + msg), overwrite);',
					'});',
					'',
					'document.addEventListener(\'DCADMIN:Ready\', function(e) {',
					'	// Use the tool',
					'	DCADMIN.tools.alertUser(\'hello world\');',
					'});',
				'</script>',
				);
				$documentation->showCode($lines, 'php'); ?>
			...
			<h3>Reading Tools</h3>
			<p>
				You can read the list of available tools using <code>DCADMIN.tools</code>. It returns a frozen copy:
			</p>

				<?php
				$lines = array(
					'<script>',
					'	console.log(DCADMIN.tools);',
					'	if(DCADMIN.checkToolExist(\'Tool name to check\')){/* ... */}else{/* ... */}; ',
					'</script>',
				);
				$documentation->showCode($lines, 'php'); ?>
		...
		<div class="documentation-section">
			...
			<p>
				The <strong>Context Vars</strong> system allows you to define and manage variables that are globally accessible within the DCADMIN JavaScript context. These variables can store configuration data, URLs, tokens, user IDs, object references, or any other values needed by your frontend code and tools.
				By using context vars, you can:
			<ul>
				<li>Pass server-side data (from PHP) to JavaScript safely and consistently.</li>
				<li>Provide reusable configuration for DCADMIN tools, widgets, or modules without hardcoding values.</li>
				...
				<li>Use <code>DCADMIN.setContextVar</code> for single values or <code>DCADMIN.setContextVars</code> to pass multiple values at once.</li>
				<li>Access these variables anywhere in your code via <code>DCADMIN.getContextVar(key)</code>.</li>
			...
			This system is particularly useful for setting up base URLs, API endpoints, user-specific information, or runtime data that needs to be shared across multiple DCADMIN frontend tools.
			</p>
			...
			<h3>Add  context var (overridable or not)</h3>
				...
				$lines = array(
					...
					'       DCADMIN.setContextVar(\'yourKey\', \'YourValue\');',
					'',
					'    	// Add overridable context var',
					'       DCADMIN.setContextVar(\'yourKey2\', \'YourValue\', true);',
					'    });',
					'</script>',
				);
				$documentation->showCode($lines, 'php');
			...
			<h3>Add multiple context vars (overridable or not)</h3>
				...
				$lines = array(
					...
					'<script nonce="<?php print getNonce() ?>" >',
					'    document.addEventListener(\'DCADMIN:Init\', function(e) {',
					'        DCADMIN.setContextVars(<?php print json_encode($contextConst); ?>);',
					'        DCADMIN.setContextVars(<?php print json_encode($contextVars); ?>, true);',
					'    });',
					'</script>',
				);
				$documentation->showCode($lines, 'php');
			...
			<h3>Get context var</h3>
				...
				$lines = array(
					'<script nonce="<?php print getNonce() ?>" >',
					'    document.addEventListener(\'DCADMIN:Ready\', function(e) {',
					'        let url = DCADMIN.getContextVar(\'DOL_URL_ROOT\', \'The optional fallback value\'));',
					'        console.log(url);',
					'    });',
					'</script>',
				);
