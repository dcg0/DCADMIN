...
{
	/** @var string Table name in SQL */
	public $table_element = 'workstation_workstation_usergroup';

	/** @var string Name of the element (tip for better integration in DCADMIN: this value should be the reflection of the class name with ucfirst() function) */
	...
	public static function getAllGroupsOfWorkstation($fk_workstation)
	{
		...
		$obj = new self($db);
		$all_items = parent::getAllItemsLinkedByObjectID($fk_workstation, 'fk_usergroup', 'fk_workstation', $obj->table_element);
		if (!is_array($all_items)) {
			return [];
		}

		return array_map('intval', $all_items);
	}
