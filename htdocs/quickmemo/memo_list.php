...
include_once __DIR__.'/class/memo.class.php';
// for other modules
...
$object = new Memo($db);
$diroutputmassaction = $conf->quickmemo->dir_output.'/temp/massgeneration/'.$user->id;
...
if (!isModEnabled("quickmemo")) {
	accessforbidden('Module quickmemo not enabled');
}
if (!$permissiontoread) {
	accessforbidden();
}
...
if (empty($reshook)) {
	...
	$objectclass = 'Memo';
	$objectlabel = 'Memo';
	$uploaddir = $conf->quickmemo->dir_output;
		...
		if (!empty($toselect)) {
			...
			foreach ($toselect as $idMemo) {
				...
				if ($selectdModel->fetch($idMemo) <= 0) {
					...
					setEventMessage($langs->trans('QuickMemoLoadMemosError').' : '. (int) $idMemo, 'errors');
					continue;
				}

				if ($selectdModel->status !== Memo::STATUS_ARCHIVED) {
				...
				if ($selectdModel->setUnArchived($user) <= 0) {
					setEventMessage($langs->trans('QuickMemoUnArchiveError').' : '. (int) $idMemo, 'errors');
					continue;
				}
			...
			setEventMessage($langs->trans('PleaseSelectAtLeastOneRow'), 'warnings');
		}
	}
}
...
$title = $langs->trans("Memos");
//$help_url = "EN:Module_Memo|FR:Module_Memo_FR|ES:Módulo_Memo";
$help_url = '';
$morejs = array();
$morecss = array(
	'quickmemo/css/memo.css'
);
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/quickmemo/memo_card.php', 1).'?id='.((int) $id));
	exit;
}
...
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'mod-quickmemo page-list bodyforlist');	// Can use also classforhorizontalscrolloftabs instead of bodyforlist for a horizontal scroll in the table instead of page
...
//$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/quickmemo/memo_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, $object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendMemoRef";
$modelmail = "memo";
$objecttmp = new Memo($db);
