...
/**
 *    \file       history_card.php
 *    \ingroup    webhook
 *    \brief      Page to create/edit/view triggerhistory
 */
	...
	if (empty($backtopage) || ($cancel && empty($id))) {
		if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
			if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
				...
				$backtopage = dol_buildpath('/webhook/history_card.php', 1).'?id='.((!empty($id) && $id > 0) ? $id : '__ID__');
			}
		}
	}

	$triggermodname = 'WEBHOOK_MYOBJECT_MODIFY'; // Name of trigger action code to execute when we modify record
...
if ($action == 'create') {
	$title = $langs->trans("NewObject", $langs->transnoentitiesnoconv("TriggerHistory"));
}
$help_url = '';

llxHeader('', $title, $help_url, '', 0, 0, '', '', '', 'mod-webhook page-card');
...
if (($id || $ref) && $action == 'edit') {
	print load_fiche_titre($langs->trans(""), '', $object->picto);
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	...
	if ($action == 'delete' || ($conf->use_javascript_ajax && empty($conf->dol_use_jmobile))) {
		$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"].'?id='.$object->id, $langs->trans('DeleteTriggereHistoryRecord'), $langs->trans('ConfirmDeleteObject'), 'confirm_delete', '', 0, 'action-delete');
	}
	...
	*/
	...
	if (!empty($object->table_element_line)) {
		...
		if (!empty($object->lines) || ($object->status == $object::STATUS_ERROR && $permissiontoadd && $action != 'selectlines' && $action != 'editline')) {
	...
	if ($action != 'presend' && $action != 'editline') {
		...
		if (empty($reshook)) {
			...
			// Back to draft
			if ($object->status == $object::STATUS_SUCCESS) {
	...
	if ($action != 'presend') {
		...
		$morehtmlcenter = dolGetButtonTitle($langs->trans('SeeAll'), '', 'fa fa-bars imgforviewmode', dol_buildpath('/webhook/history_agenda.php', 1).'?id='.$object->id);
