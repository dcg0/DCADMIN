...
class Expedition extends CommonObject
{
	...
	public $meths;
	/**
	 * @var array<int,array{rowid:int,code:string,libelle:string,description:string,tracking:string,active:int}>
	 */
					...
					for ($i = 0; $i < $num; $i++) {
						...
						if (empty($this->lines[$i]->product_type) || getDolGlobalString('STOCK_SUPPORTS_SERVICES') || getDolGlobalString('SHIPMENT_SUPPORTS_SERVICES')) {
							...
							if ($line->fk_product > 0) {
								// We group kit lines by the fulfilled order line (fk_elementdet) and not by product id: when a same kit product
								// is present on several distinct order lines, each order line must keep its own shipment line so the ordered
								// qty stays consistent with the shipped qty (grouping by product would sum the shipped qty of all lines while
								// keeping the ordered qty of a single line, resulting in a wrong/negative remaining qty to ship). Several
								// warehouse pickings of the same order line still share the same fk_elementdet and are grouped together.
								if (!isset($kits_list[$line->fk_elementdet])) {
									...
									if (count($prods_arbo) > 0) {
										$kits_list[$line->fk_elementdet] = array(
											'arbo' => $prods_arbo,
											'total_qty' => $line->qty,
										);
									}
								} else {
									$kits_list[$line->fk_elementdet]['total_qty'] += $line->qty;
								}
							}
						}
					}
					...
					if (empty($line->product_type) || getDolGlobalString('STOCK_SUPPORTS_SERVICES') || getDolGlobalString('SHIPMENT_SUPPORTS_SERVICES')) {
						...
						if (!isset($kits_id_cached[$line->fk_elementdet])) {
							if (!isset($line->detail_batch) || (isset($kits_list[$line->fk_elementdet]) && !getDolGlobalInt('PRODUIT_SOUSPRODUITS_ALSO_ENABLE_PARENT_STOCK_MOVE'))) {    // no batch management or is kit
								$qty = isset($kits_list[$line->fk_elementdet]) ? $kits_list[$line->fk_elementdet]['total_qty'] : $line->qty;
								$warehouse_id = (isset($kits_list[$line->fk_elementdet]) && !getDolGlobalInt('PRODUIT_SOUSPRODUITS_ALSO_ENABLE_PARENT_STOCK_MOVE')) ? 0 : $line->entrepot_id;
								$line_id = $this->create_line($warehouse_id, $line->origin_line_id, $qty, $line->rang, $line->array_options, 0, $line->fk_product);
								if ($line_id <= 0) {
									$error++;
								}
								if (isset($kits_list[$line->fk_elementdet])) {
									$kits_id_cached[$line->fk_elementdet] = $line_id;
								}
							...
							$line_id = $kits_id_cached[$line->fk_elementdet];
						}

						// virtual products
						if (isset($kits_list[$line->fk_elementdet]) && !getDolGlobalInt('PRODUIT_SOUSPRODUITS_ALSO_ENABLE_PARENT_STOCK_MOVE')) {
							$prods_arbo = $kits_list[$line->fk_elementdet]['arbo'];
							...
							foreach ($prods_arbo as $index => $product_child_arr) {
								...
								if ($product_child_level != $level_last) {
									...
									if (isset($sub_parents_cached[$line->fk_elementdet][$parent_product_id])) {
										$parent_line_id = $sub_parents_cached[$line->fk_elementdet][$parent_product_id];
									} else {
										$sub_parents_cached[$line->fk_elementdet][$parent_product_id] = $parent_line_id;
									}
								}
								...
								if (!isset($sub_kits_id_cached[$line->fk_elementdet][$product_child_id]) || $warehouse_id > 0) {
									...
									if (empty($warehouse_id)) {
										$sub_kits_id_cached[$line->fk_elementdet][$product_child_id] = $line_id;
									}
								}
	...
	public function valid($user, $notrigger = 0)
	{
		...
		if (!$error && isModEnabled('stock') && getDolGlobalString('STOCK_CALCULATE_ON_SHIPMENT')) {
			$result = $this->manageStockMvtOnEvt($user, "ShipmentValidatedInDCADMIN");
			if ($result < 0) {
				$error++;
			}
		}
	 ...
	 * @param	array<string,mixed>		$array_options		extrafields array
	 * @param	CommonObjectLine		$origin_line		Origin line (only from OrderLine at this moment)
	 * @return	int						Return integer <0 if KO, >0 if OK
	 */
				...
				for ($i = 0; $i < $cpt; $i++) {
					...
					if ($can_update_stock && (empty($obj->iskit) || getDolGlobalInt('PRODUIT_SOUSPRODUITS_ALSO_ENABLE_PARENT_STOCK_MOVE')) && !empty($obj->incdec)) {
						...
						if (empty($lotArray)) {
							...
							$result = $mouvS->reception($user, $obj->fk_product, $obj->fk_entrepot, $obj->qty, 0, $langs->trans("ShipmentCanceledInDCADMIN", $this->ref), '', '', '', '', 0, '', 0, (empty($obj->iskit) ? 1 : 0)); // Price is set to 0, because we don't want to see WAP changed
							if ($result < 0) {
								$error++;
							...
							foreach ($lotArray as $lot) {
								$result = $mouvS->reception($user, $obj->fk_product, $obj->fk_entrepot, $lot->qty, 0, $langs->trans("ShipmentCanceledInDCADMIN", $this->ref), $lot->eatby, $lot->sellby, (string) $lot->batch, '', 0, '', 0, (empty($obj->iskit) ? 1 : 0)); // Price is set to 0, because we don't want to see WAP changed
								if ($result < 0) {
									$error++;
									$this->errors = array_merge($this->errors, $mouvS->errors);
									break;
				...
				for ($i = 0; $i < $cpt; $i++) {
					...
					if ($can_update_stock && (empty($obj->iskit) || getDolGlobalInt('PRODUIT_SOUSPRODUITS_ALSO_ENABLE_PARENT_STOCK_MOVE')) && !empty($obj->incdec)) {
						...
						if (empty($lotArray)) {
							...
							// We use warehouse selected for each line
							$result = $mouvS->reception($user, $obj->fk_product, $obj->fk_entrepot, $obj->qty, 0, $langs->trans("ShipmentDeletedInDCADMIN", $this->ref), '', '', '', '', 0, '', 0, (empty($obj->iskit) ? 1 : 0)); // Price is set to 0, because we don't want to see WAP changed
							if ($result < 0) {
								$error++;
								$this->errors = array_merge($this->errors, $mouvS->errors);
								break;
							...
							foreach ($lotArray as $lot) {
								...
								$result = $mouvS->reception($user, $obj->fk_product, $obj->fk_entrepot, $lot->qty, 0, $langs->trans("ShipmentDeletedInDCADMIN", $this->ref), $lot->eatby, $lot->sellby, (string) $lot->batch, '', 0, '', 0, (empty($obj->iskit) ? 1 : 0)); // Price is set to 0, because we don't want to see WAP changed
								if ($result < 0) {
									$error++;
									$this->errors = array_merge($this->errors, $mouvS->errors);
									break;
	...
	public function fetch_delivery_methods()
	{
		...
		if ($resql) {
			while ($obj = $this->db->fetch_object($resql)) {
				...
				$this->meths[(int) $obj->rowid] = ($label != 'SendingMethod'.$obj->code ? $label : $obj->label);
			}
		}
	}
	...
	public function getUrlTrackingStatus($value = '')
	{
		...
		if (!empty($tracking) && !empty($value)) {
			...
			$this->tracking_url = sprintf('<a target="_blank" rel="noopener noreferrer" href="%s">%s</a>', $url, $value);
		} else {
	 ...
	 * @throws Exception
	 */
	private function manageStockMvtOnEvt($user, $labelmovement = 'ShipmentClassifyClosedInDCADMIN', $reverse = false)
	{
		global $langs;
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		if (isModEnabled('stock') && getDolGlobalString('STOCK_CALCULATE_ON_SHIPMENT')) {
			...
			$result = $this->manageStockMvtOnEvt($user, "ShipmentBackToDraftInDCADMIN", true);
			if ($result < 0) {
				$error++;
			}
		}
				...
				if ($resql) {
					...
					for ($i = 0; $i < $cpt; $i++) {
						...
						if (empty($obj->edbrowid)) {
							...
							// We decrement stock of product (and sub-products) -> update table llx_product_stock (key of this table is fk_product+fk_entrepot) and add a movement record
							$result = $mouvS->livraison($user, $obj->fk_product, $obj->fk_entrepot, -$qty, $obj->subprice, $langs->trans("ShipmentUnClassifyCloseddInDCADMIN", $this->ref));
							if ($result < 0) {
								$this->setErrorsFromObject($mouvS);
								$error++;
								break;
							...
							// We decrement stock of product (and sub-products) -> update table llx_product_stock (key of this table is fk_product+fk_entrepot) and add a movement record
							$result = $mouvS->livraison($user, $obj->fk_product, $obj->fk_entrepot, -$qty, $obj->subprice, $langs->trans("ShipmentUnClassifyCloseddInDCADMIN", $this->ref), '', $this->db->jdate($obj->eatby), $this->db->jdate($obj->sellby), $obj->batch, $obj->fk_origin_stock);
							if ($result < 0) {
								$this->setErrorsFromObject($mouvS);
								$error++;
								break;
