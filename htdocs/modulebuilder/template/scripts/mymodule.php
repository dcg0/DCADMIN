...
// Test if batch mode
if (substr($sapi_type, 0, 3) == 'cgi') {  // @phan-suppress-current-line DCADMINForbiddenFunctionPlugin
...
if (!$res && $i > 0 && file_exists(substr($tmp, 0, ($i + 1))."/master.inc.php")) {
	$res = @include substr($tmp, 0, ($i + 1))."/master.inc.php";  // @phan-suppress-current-line DCADMINForbiddenFunctionPlugin
}
if (!$res && $i > 0 && file_exists(dirname(substr($tmp, 0, ($i + 1)))."/master.inc.php")) {  // @phan-suppress-current-line DCADMINForbiddenFunctionPlugin
	$res = @include dirname(substr($tmp, 0, ($i + 1)))."/master.inc.php";  // @phan-suppress-current-line DCADMINForbiddenFunctionPlugin
}
