...
pHeader($langs->trans("DCADMINSetup").' - '.$langs->trans("ConfigurationFile"), "step2");
...
if (!$error) {
	...
	if ($result) {
		...
		if (!empty($db_create_database) && !$userroot) {
			print '<div class="error">'.$langs->trans("YouAskDatabaseCreationSoDCADMINNeedToConnect", $db_name).'</div>';
			print '<br>';
			print $langs->trans("BecauseConnectionFailedParametersMayBeWrong").'<br><br>';
			print $langs->trans("ErrorGoBackAndCorrectParameters");
			$error++;
		...
		if (!empty($db_create_user) && !$userroot) {
			print '<div class="error">'.$langs->trans("YouAskLoginCreationSoDCADMINNeedToConnect", $db_user).'</div>';
			print '<br>';
			print $langs->trans("BecauseConnectionFailedParametersMayBeWrong").'<br><br>';
			print $langs->trans("ErrorGoBackAndCorrectParameters");
			$error++;
		...
		if (isset($db_create_user) && ($db_create_user == "1" || $db_create_user == "on")) {
			...
			if (!$error && $db !== null) {
				if ($db->connected) {
					...
					print $langs->trans("YouAskDatabaseCreationSoDCADMINNeedToConnect", $dolibarr_main_db_user, $dolibarr_main_db_host, $userroot);
					print '<br>';
					print $langs->trans("BecauseConnectionFailedParametersMayBeWrong").'<br><br>';
					print $langs->trans("ErrorGoBackAndCorrectParameters").'<br><br>';
					print '</td></tr>';
...
function write_conf_file($conffile)
{
	...
	if ($fp) {
		...
		fwrite($fp, '<?php'."\n");
		fwrite($fp, '//'."\n");
		fwrite($fp, '// File generated by DCADMIN installer '.DOL_VERSION.' on '.dol_print_date(dol_now(), '')."\n");
		fwrite($fp, '//'."\n");
		...
		fwrite($fp, '$dolibarr_main_restrict_os_commands=\'mariadb-dump, mariadb, mysqldump, mysql, pg_dump, pg_restore, clamdscan, clamdscan.exe, ls, tar, gzip, bz2, zstd\';');
		fwrite($fp, "\n");
