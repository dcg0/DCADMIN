...
if ($action == 'addcontainer' && $usercanedit) {
			...
			if (!$error) {
				...
				foreach ($regs[0] as $key => $val) {
					...
					if (!empty($tmpgeturl['curl_error_no'])) {
						...
						// We try to convert the CSS we got by adding a prefix .bodywebsite with lessc to avoid conflict with CSS of DCADMIN.
				...
				// Normalize links href to DCADMIN internal naming
	...
	if (!$error) {
		...
		if (!dol_is_file($filerobot)) {
			$robotcontent = "# Robot file. Generated with DCADMIN\nUser-agent: *\nAllow: /public/\nDisallow: /administrator/";
		...
		if (!dol_is_file($filereadme)) {
			$readmecontent = "Website generated by DCADMIN ERP CRM";
...
if (!GETPOST('hide_websitemenu')) {
	...
	if ($websitekey && $websitekey != '-1' && (!in_array($action, array('editcss', 'editmenu', 'importsite', 'file_manager', 'replacesite', 'replacesiteconfirm', 'editsecurity'))) && (!in_array($mode, array('replacesite'))) && !$file_manager) {
		...
		if (($pageid > 0 && $atleastonepage) && ($action == 'preview' || $action == 'createfromclone' || $action == 'createpagefromclone' || $action == 'deletesite')) {
			...
			$htmltext = $langs->trans("PreviewSiteServedByDCADMIN", $langs->transnoentitiesnoconv("Page"), $langs->transnoentitiesnoconv("Page"), $realpage, $langs->transnoentitiesnoconv("TestDeployOnWeb"));
...
if ((empty($action) || $action == 'preview' || $action == 'createfromclone' || $action == 'createpagefromclone') && !in_array($mode, array('replacesite'))) {
	if ($pageid > 0 && $atleastonepage) {
		...
		// Output page under the DCADMIN top menu
		...
		*/
		...
		// Clean style that may affect global style of DCADMIN
