		...
		if (!$error) {
			...
			if ($object->email && $sendalsoemail) {   // $object is 'Adherent'
				...
				if (empty($reshook)) {
					...
					$moreinheader = 'X-DCADMIN-Info: send_an_email by adherents/subscription.php'."\r\n";
...
if (isModEnabled('societe')) {
	...
	print $langs->trans("LinkedToDCADMINThirdParty");
	print '</td>';
...
print '<tr><td>';
print '<table class="nobordernopadding centpercent"><tr><td>';
print $langs->trans("LinkedToDCADMINUser");
print '</td>';
...
if (($action == 'createsubscription' || $action == 'create_thirdparty') && $user->hasRight('adherent', 'cotisation', 'creer')) {
	...
	if ($action == 'create_thirdparty') {
		...
		print $form->formconfirm($_SERVER["PHP_SELF"]."?rowid=".$object->id, $langs->trans("CreateDCADMINThirdParty"), $langs->trans("ConfirmCreateThirdParty"), "confirm_create_thirdparty", $formquestion, 1);
	}
		...
		if ((isModEnabled('bank') || isModEnabled('invoice')) && !getDolGlobalString('ADHERENT_SUBSCRIPTION_HIDECOMPLEMENTARYACTIONS')) {
			...
			if (isModEnabled('societe') && isModEnabled('invoice')) {
				...
				if ($object->socid) {
					...
					print $langs->trans("CreateDCADMINThirdParty");
					print '</a>)';
				}
