...
/**
 *	    \file       htdocs/contrat/stats/index.php
 *      \ingroup    contrat
...
require_once DOL_DOCUMENT_ROOT.'/contrat/class/contratstats.class.php';
...
$langs->loadLangs(array('contracts', 'companies', 'other', 'suppliers'));
...
if (!$user->hasRight('societe', 'client', 'voir')) {
	$filenamenb = $dir.'/contractsnbinyear-'.$user->id.'-'.$year.'.png';
	$fileurlnb = DOL_URL_ROOT.'/viewimage.php?modulepart=contractstats&file=contractsnbinyear-'.$user->id.'-'.$year.'.png';
} else {
	$filenamenb = $dir.'/contractsnbinyear-'.$year.'.png';
	$fileurlnb = DOL_URL_ROOT.'/viewimage.php?modulepart=contractstats&file=contractsnbinyear-'.$year.'.png';
}
...
if (!$mesg) {
	...
	$px1->SetYLabel($langs->trans("NbOfContracts"));
	$px1->SetShading(3);
	$px1->SetHorizTickIncrement(1);
	$px1->mode = 'depth';
	$px1->SetTitle($langs->trans("NumberOfContractsByMonth"));
...
$fileurlamount = '';
if (!$user->hasRight('societe', 'client', 'voir')) {
	$filenameamount = $dir.'/contractsamountinyear-'.$user->id.'-'.$year.'.png';
	$fileurlamount = DOL_URL_ROOT.'/viewimage.php?modulepart=contractstats&file=contractsamountinyear-'.$user->id.'-'.$year.'.png';
} else {
	$filenameamount = $dir.'/contractsamountinyear-'.$year.'.png';
	$fileurlamount = DOL_URL_ROOT.'/viewimage.php?modulepart=contractstats&file=contractsamountinyear-'.$year.'.png';
}
...
if (!$mesg) {
	...
	$px2->SetYLabel($langs->trans("AmountOfServices"));
	$px2->SetShading(3);
	$px2->SetHorizTickIncrement(1);
	$px2->mode = 'depth';
	$px2->SetTitle($langs->trans("AmountOfServicesByMonthHT"));
...
$h = 0;
$head = array();
$head[$h][0] = DOL_URL_ROOT.'/contrat/stats/index.php?mode='.$mode;
...
print '<td class="right">'.$langs->trans("NbOfContracts").'</td>';
