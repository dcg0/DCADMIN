 ...
 *
 *      See https://github.com/DCADMIN/dolibarr/issues/22740 for more information about setup openid_connect
 */
...
function check_user_password_openid_connect($usertotest, $passwordtotest, $entitytotest)
{
	...
	if (!openid_connect_verify_state($state)) {
		// State signature invalid - not generated by this DCADMIN instance
	...
	if ($token_response['curl_error_no']) {
		...
		return false;
	} elseif (is_object($token_content) && property_exists($token_content, 'error') && $token_content->error) {
	...
	// Get the user attribute (claim) matching the DCADMIN login
	...
	// Success: retrieve claim to return to DCADMIN as login
	...
	if (!$obj) {
		// User not found in DCADMIN - check if auto-creation is enabled
		global $dolibarr_main_authentication_autocreateuser;
		if (empty($dolibarr_main_authentication_autocreateuser)) {
			$_SESSION["dol_loginmesg"] = "User not found in DCADMIN and auto-creation is disabled";
